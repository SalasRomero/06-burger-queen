...
import { QuantityProduct } from '../models/quantitryProduct';
...
export class UserOrderService {
  ...
  numProducts(){
    if(this.order && this.order.products.length > 0){
      return this.order.products.reduce((acum:number,value:QuantityProduct) => 
  ...
  async oneMoreProduct(product:Product){
    const productFound = await this.searchProduct(product);
  ...
  totalOrder(){
    let total = 0;
    for(let quantityProduct of this.order.products){
